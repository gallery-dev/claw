 ...
 */
async function runQuery(prompt, sessionId, mcpServerPath, containerInput, sdkEnv, activityPoster, resumeAt) {
...
async function main() {
    ...
    const activityPoster = new ActivityPoster(containerInput.secrets || {}, containerInput.assistantName || containerInput.groupFolder);
    ...
    if (browserProfileDir && fs.existsSync(browserProfileDir)) {
        for (const lockFile of ['SingletonLock', 'SingletonSocket', 'SingletonCookie']) {
            try {
                fs.unlinkSync(path.join(browserProfileDir, lockFile));
            }
            catch { /* ignore */ }
        }
    }
