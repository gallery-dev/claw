...
server.tool('register_group', `Register a new messaging group so the agent can respond to messages there. Main group only.

Use available_groups.json to find the JID for a group. The folder name should be lowercase with hyphens (e.g., "family-chat").`, {
    jid: z.string().describe('The group JID or chat ID (e.g., "120363336345536173@g.us")'),
...
server.tool('memory_view', `View memory directory contents or read a specific memory file. Use this to check what you've remembered from past sessions.
...
Returns directory listing (with sizes) or file contents with line numbers.`, {
    ...
    if (requestedPath === 'MEMORY.md' || requestedPath === '/MEMORY.md') {
        ...
        if (entries.length === 0) {
            return { content: [{ type: 'text', text: 'Memory is empty. Use memory_write to save notes.' }] };
        }
        return { content: [{ type: 'text', text: `Memory files:\n${entries.join('\n')}` }] };
    }
    else {
    ...
    if (!fs.existsSync(targetPath)) {
        return { content: [{ type: 'text', text: `No memory file at "${args.path}". Use memory_write to create one.` }] };
    }
    ...
    if (args.view_range) {
        ...
        return { content: [{ type: 'text', text: `${args.path} (lines ${start}-${end}):\n${numbered}` }] };
    }
    const numbered = lines.map((line, i) => `${String(i + 1).padStart(6)}\t${line}`).join('\n');
    return { content: [{ type: 'text', text: `${args.path}:\n${numbered}` }] };
});
server.tool('memory_write', `Write or update a memory file. Use this to persist important information across sessions.
...
When updating, prefer str_replace mode to avoid overwriting existing content.`, {
    ...
    if (args.mode === 'create' && fs.existsSync(targetPath)) {
        return { content: [{ type: 'text', text: `Error: "${args.path}" already exists. Use mode "append" or "replace".` }], isError: true };
    }
    if (args.mode === 'append') {
...
server.tool('memory_search', `Search across all memory files for relevant information. Uses keyword matching across MEMORY.md, memory/*.md, and conversations/*.md.
...
Returns matching snippets with file paths and line numbers. Use this when you need to find something but don't know which file it's in.`, {
    ...
    const searchFile = (filePath, displayName) => {
        try {
    ...
    const searchDir = (dirPath, prefix) => {
    ...
    if (results.length === 0) {
        return { content: [{ type: 'text', text: `No matches for "${args.query}" in ${args.scope} files.` }] };
    }
    ...
    return { content: [{ type: 'text', text: `Found ${deduped.length} match(es) for "${args.query}":\n\n${formatted}${truncated}` }] };
});
server.tool('memory_delete', 'Delete a memory file that is no longer relevant. Keeps memory organized.', {
    ...
    if (!fs.existsSync(targetPath)) {
        return { content: [{ type: 'text', text: `File "${args.path}" not found.` }], isError: true };
    }
    fs.unlinkSync(targetPath);
    return { content: [{ type: 'text', text: `Deleted ${args.path}` }] };
});
// ─── Multi-Session Management Tools ──────────────────────
server.tool('list_sessions', 'List all active agent sessions across groups. Main group only. Shows group name, folder, and container status.', {}, async () => {
    if (!isMain) {
        return { content: [{ type: 'text', text: 'Only the main group can list all sessions.' }], isError: true };
    }
    const sessionsFile = path.join(IPC_DIR, 'active_sessions.json');
    try {
        if (!fs.existsSync(sessionsFile)) {
            return { content: [{ type: 'text', text: 'No session data available. Sessions are populated before each agent run.' }] };
        }
        const sessions = JSON.parse(fs.readFileSync(sessionsFile, 'utf-8'));
        if (!Array.isArray(sessions) || sessions.length === 0) {
            return { content: [{ type: 'text', text: 'No groups registered.' }] };
        }
        const formatted = sessions
            .map((s) => `- **${s.groupName}** (${s.groupFolder}): ${s.hasActiveContainer ? 'container active' : 'idle'}${s.hasSession ? ', has session' : ''}`)
            .join('\n');
        return { content: [{ type: 'text', text: `Active sessions:\n${formatted}` }] };
    }
    catch (err) {
        return { content: [{ type: 'text', text: `Error reading sessions: ${err instanceof Error ? err.message : String(err)}` }] };
    }
});
server.tool('send_to_group', 'Send a message to another group\'s agent. Main group only. The message will be queued as if a user sent it, triggering the target agent to process it.', {
    ...
    if (!isMain) {
        return { content: [{ type: 'text', text: 'Only the main group can send messages to other groups.' }], isError: true };
    }
    const data = {
        type: 'send_to_group',
    ...
    writeIpcFile(TASKS_DIR, data);
    return { content: [{ type: 'text', text: `Message queued for group "${args.target_group_folder}".` }] };
});
server.tool('close_group_session', 'Close another group\'s active agent container. Main group only. Use to free resources or restart a stuck agent.', {
    target_group_folder: z.string().describe('The target group folder name to close'),
}, async (args) => {
    if (!isMain) {
        return { content: [{ type: 'text', text: 'Only the main group can close other group sessions.' }], isError: true };
    }
    const data = {
    ...
    writeIpcFile(TASKS_DIR, data);
    return { content: [{ type: 'text', text: `Close request sent for group "${args.target_group_folder}".` }] };
});
