...
// ─── Messaging Tools ──────────────────────────────────
server.tool('send_message', "Send a message to the user or group immediately while you're still running. Use this for progress updates or to send multiple messages. Note: when running as a scheduled task, your final output is NOT sent to the user — use this tool if you need to communicate.", {
    text: z.string().describe('The message text to send'),
    sender: z.string().optional().describe('Your role/identity name (e.g. "Researcher").'),
}, async (args) => {
    if (!galleryApiUrl || !galleryToken) {
        return {
            content: [{ type: 'text', text: 'Message sent (Gallery API not configured — message logged locally).' }],
        };
    }
    try {
        ...
        if (!response.ok) {
            return {
                content: [{ type: 'text', text: `Message delivery failed: ${response.status} ${response.statusText}` }],
                isError: true,
            };
        }
        ...
        return {
            content: [{ type: 'text', text: `Message delivery error: ${err instanceof Error ? err.message : String(err)}` }],
            isError: true,
        };
    }
});
...
• once: Local time (e.g., "2026-03-01T15:30:00")`, {
    ...
    return {
        content: [{ type: 'text', text: 'Task scheduling is being migrated to Gallery. This feature will be available soon. For now, please ask the user to set up scheduled tasks in the Gallery dashboard.' }],
    };
});
server.tool('list_tasks', "List all scheduled tasks.", {}, async () => {
    return {
        content: [{ type: 'text', text: 'Task listing is being migrated to Gallery. Please check the Gallery dashboard for scheduled tasks.' }],
    };
});
server.tool('pause_task', 'Pause a scheduled task.', { task_id: z.string().describe('The task ID to pause') }, async () => {
    return {
        content: [{ type: 'text', text: 'Task management is being migrated to Gallery. Please use the Gallery dashboard.' }],
    };
});
server.tool('resume_task', 'Resume a paused task.', { task_id: z.string().describe('The task ID to resume') }, async () => {
    return {
        content: [{ type: 'text', text: 'Task management is being migrated to Gallery. Please use the Gallery dashboard.' }],
    };
});
server.tool('cancel_task', 'Cancel and delete a scheduled task.', { task_id: z.string().describe('The task ID to cancel') }, async () => {
    return {
        content: [{ type: 'text', text: 'Task management is being migrated to Gallery. Please use the Gallery dashboard.' }],
    };
});
...
server.tool('memory_delete', 'Delete a memory file that is no longer relevant.', {
